...
int main() {
    ...
    return_value = bind(server_socket, (struct sockaddr *) &server_sockaddr, sizeof(server_sockaddr));
    if (return_value == FAIL) {
        perror("=== Error in bind");
        goto FINISH;
    }
    return_value = listen(server_socket, MAX_CLIENTS_COUNT);
    if (return_value == FAIL) {
    ...
    FINISH:
    {
        return_value = close(server_socket);
        if (return_value == FAIL) {
            perror("=== Error in close");
        }
        return EXIT_SUCCESS;
