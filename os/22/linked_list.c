...
list_node_t *pop_head_list_node(list_t *list) {
    assert(list);
    if (0 == list->len) {
        return NULL;
    }
...
list_node_t *pop_tail_list_node(list_t *list) {
    assert(list);
    if (0 == list->len) {
        return NULL;
    }
...
void *pop_head(list_t *list) {
    assert(list);
    list_node_t *list_node = pop_head_list_node(list);
    if (NULL == list_node) {
        return NULL;
    }
...
void *pop_tail(list_t *list) {
    assert(list);
    list_node_t *list_node = pop_tail_list_node(list);
    if (NULL == list_node) {
        return NULL;
    }
    void *value = list_node->value;
...
list_node_t *get_next_node(const list_t *list, const list_node_t *current) {
    assert(list);
    assert(current);
